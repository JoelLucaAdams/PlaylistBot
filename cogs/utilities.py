...
class Utilities(commands.Cog):
    ...
    async def source(self, ctx: Context):
        ...
        await ctx.send(content='Created by `Joel Adams`\n'
                               'https://github.com/JoelLucaAdams/PlaylistBot')
...
class Youtube(commands.Cog):
    ...
    async def add(self, ctx: Context, playlist_index: int, yt_link: str):
        ...
        embed.add_field(name='⏱️ Song Length', value=f'{formatted_video_time}', inline=False)
    ...
    async def playlists(self, ctx: Context):
        ...
        for item in Playlists:
            ...
            embed.add_field(name='⏱️ Playlist Length', value=f'{playlist_length}', inline=False)
            i += 1
        await ctx.send(embed=embed)